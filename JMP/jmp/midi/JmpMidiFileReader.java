 ...
 */
public final class JmpMidiFileReader extends MidiFileReader {
    ...
    public Sequence getSequence(InputStream stream) throws InvalidMidiDataException, IOException {
        ...
        for (int i = 0; i < smfParser.tracks; i++) {
            if (smfParser.nextTrack()) {
                short trackIndex = (short)sequence.getTracks().length;
                smfParser.readTrack(sequence.createTrack(), trackIndex);
            } else {
                break;
            }
        }
...
final class SMFParser {
    ...
    void readTrack(Track track) throws IOException, InvalidMidiDataException {
        readTrack(track, (byte)0);
    }
    void readTrack(Track track, short currentTrackIndex) throws IOException, InvalidMidiDataException {
        try {
            ...
            while (!trackFinished() && !endOfTrackFound) {
                ...
                case 0xE0:
                    ...
                    message = new LightweightShortMessage(status | (data1 << 8) | (data2 << 16), currentTrackIndex);
                    break;
                case 0xC0:
                case 0xD0:
                    // one data byte
                    ...
                    message = new LightweightShortMessage(status | (data1 << 8), currentTrackIndex);
                    break;
                case 0xF0:
                    // sys-ex or meta
                    switch(status) {
